...
public class E14_19 extends Application {
  @Override
  public void start(Stage primaryStage) {
    ...
    list = cosLine.getPoints();
    for (int x = -200; x <= 200; x++) {
      list.add(x + 200.0);
      list.add((WINDOW / 5 * 4) - scaleFactor * Math.cos((x / 100.0) * 2 * Math.PI));
    }

    graphPane.getChildren().addAll(sinLine, cosLine);
